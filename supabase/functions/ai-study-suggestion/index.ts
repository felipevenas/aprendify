...
serve(async (req) => {
  ...
  try {
    ...
    let rateLimit;
    try {
      rateLimit = await consumeRateLimit(supabaseService, req, user.id, "ai-study-suggestion", RATE_LIMIT_MAX_CALLS, RATE_LIMIT_WINDOW_MINUTES);
    } catch (error) {
      console.error("[ai-study-suggestion] Rate limit unavailable:", error instanceof Error ? error.name : "unknown");
