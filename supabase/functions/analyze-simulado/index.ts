...
serve(async (req) => {
  ...
  try {
    ...
    let rateLimit;
    try {
      rateLimit = await consumeRateLimit(supabaseRateLimit, req, user.id, "analyze-simulado", RATE_LIMIT_MAX_CALLS, RATE_LIMIT_WINDOW_MINUTES);
    } catch (error) {
      console.error("[analyze-simulado] Rate limit unavailable:", error instanceof Error ? error.name : "unknown");
    ...
    if (!rateLimit.allowed) {
      return new Response(JSON.stringify({ error: "Limite de análises atingido. Tente novamente em 1 hora.", code: "RATE_LIMITED", rateLimited: true }), {
        status: 429,
    ...
    try {
      premiumAccess = await hasPremiumAccess(supabaseRateLimit, user.id);
    } catch {
